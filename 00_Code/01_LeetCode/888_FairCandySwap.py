...
class Solution:
    def fairCandySwap(self, A, B):
        ...
        * The first thought that comes to mind is a naive
        brute force O(n^2) solution, that involves looping
        ...
        * We need to find a pair (a,b) such that a=b+diff
        
        Your runtime beats 95.33 % of python3 submissions.
        """
        var_diff = (sum(A) - sum(B)) // 2

        A = set(A)

        for ele_b in set(B):
            if var_diff + ele_b in A:
                return [var_diff + ele_b, ele_b]